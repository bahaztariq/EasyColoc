{{-- Add Category Modal --}}
<div x-data="{ open: false }" @toggle-category.window="open = !open">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div class="px-6 pt-6 pb-4 border-b border-gray-100">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-purple-500 flex items-center justify-center shadow-sm">
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-gray-900">Add Category</h3>
                            <p class="text-xs text-gray-500">Organize your expenses</p>
                        </div>
                    </div>
            ...
            <form method="POST" action="{{ route('categories.store') }}" class="p-6 space-y-5">
                @csrf
                <div>
                    <label for="category_name" class="block text-sm font-semibold text-gray-700 mb-1.5">Category Name</label>
                    <input type="text" name="name" id="category_name" value="{{ old('name') }}" placeholder="e.g. Groceries, Rent, Utilities"
                        class="block w-full border border-gray-200 rounded-xl py-2.5 px-4 text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all duration-200 placeholder-gray-400" required>
                    @error('name')
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit"
                        class="px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-violet-500 to-purple-500 hover:from-violet-600 hover:to-purple-600 rounded-xl shadow-sm shadow-violet-200 transition-all duration-200 hover:shadow-md active:scale-95">
                        Add Category
                    </button>
                </div>
            </form>
